...
import { ITicket, readAllTickets } from '../tickets/TicketActions';
...
const TicketVerify: FC = (): ReactElement => {
  ...
  const handleActionClick = (type: string, id: string = ''): void => {
    const url = `/tickets/verify/form${id ? `?id=${id}&` : '?'}type=${type}`;
    ...
    <Box>
      <Box display="flex" alignItems="center">
        <Box display="flex" alignItems="center" justifyContent="flex-end" width="100%">
          <CustomButton buttonText="Verify Ticket" onClick={() => handleActionClick('verify')} />
        </Box>
      </Box>
      {isEmpty(tickets) ? (
